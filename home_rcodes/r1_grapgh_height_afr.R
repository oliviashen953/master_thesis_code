...
pgs_methods <- c("real_african_ols_rank_no", "PGS002801_rank", "pgs_average_rank")
...
method_colors <- c(
  ...
  "Supervised_Best: PGS002801" = "#984EA3", # Purple
...
plot <- ggplot(filtered_methods, aes(x = group_percentile, y = mean_standardized_difference, group = rank_method,
  ...
  # title = "Height (Supervised AFR3000): Comparison of Rank Methods Across Top Percentiles") +
  guides(color = guide_legend(override.aes = list(shape = c(19, 17, 15, 18, 16, 13))))


plot
ggsave("r1_Height_AFR_Top_Percentiles.png", plot = plot, width = 10, height = 8, dpi = 300)