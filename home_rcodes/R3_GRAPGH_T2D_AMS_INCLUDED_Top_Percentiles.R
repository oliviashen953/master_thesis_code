...
R3_T2D_summary_odds_ratio_and_rr_across_groups <- read_csv("R3_T2D_summary_odds_ratio_and_rr_across_groups.csv")
View(R3_T2D_summary_odds_ratio_and_rr_across_groups)

library(readr)
T2D_summary <- R3_T2D_summary_odds_ratio_and_rr_across_groups


ra_methods <- c("mc4_rank", "logs_rank", "ams_rank" , "RA_average_rank")
pgs_methods <- c("supervise3000_ols_rank_no", "PGS000806_rank", "pgs_average_rank")
...
pgs_labels <- setNames(c(
  ...
  "Supervised3000 Best PGS Model: PGS000806",
  "Average PGS Rank"
), all_methods)
...
method_colors <- c(
  ...
  "Supervised3000 Best PGS Model: PGS000806" = "#984EA3", # Purple
...
plot
ggsave("R3_T2D_AMS_INCLUDED_Top_Percentiles.png", plot = plot, width = 10, height = 8, dpi = 300)
