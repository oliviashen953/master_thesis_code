...
library(readr)
Sorted_Summary_Height_averages_pheno_by_percentile <- read_csv("SORTED_Summary_Height_averages_pheno_by_percentile.csv")
View(Sorted_Summary_Height_averages_pheno_by_percentile)

Height_TOTOAL_results <- Sorted_Summary_Height_averages_pheno_by_percentile


library(dplyr)


pgs_benchmarks <- c("real_african_ols","pgs_average", "PGS002801")
ra_methods <- c("mc4")
...
filtered_methods <- Height_TOTOAL_results %>%
  filter(rank_method %in% all_methods)


pgs_labels <- c(
  "real_african_ols" = "supervised OLS 3000 training (supervise AFR3000)",
  "PGS002801" = "supervised best 3000 training: PGS002801 (supervise AFR3000)",
...
ggplot(top_percentiles, aes(x = percentile, y = mean_average_pheno, group = rank_method,
  ...
  theme_bw() +
  labs(y = "Average Height (in cm)", x = "Top Catching Percentiles",
       title = "Height: Comparison of Best RA Method with PGS Models for Top percentiles (supervise AFR3000)") +
  ylim(y_min, y_max) 

ggsave("R1_AFR_Height_Top_Percentiles.png", width = 10, height = 8, dpi = 300)
