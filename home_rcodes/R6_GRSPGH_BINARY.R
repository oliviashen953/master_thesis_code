...
R6_T2D_summary_odds_ratio_and_rr_across_groups <- read_csv("R6_T2D_summary_odds_ratio_and_rr_across_groups.csv")
View(R6_T2D_summary_odds_ratio_and_rr_across_groups)


library(readr)
T2D_summary <- R6_T2D_summary_odds_ratio_and_rr_across_groups
...
pgs_labels <- setNames(c(
  ...
  "Supervised3000 Best PGS Model: PGS002308",
  "Average PGS Rank"
), all_methods)
...
method_colors <- c(
  ...
  "Supervised3000 Best PGS Model: PGS002308" = "#984EA3", # Purple
...
plot <- ggplot(filtered_methods, aes(x = percentile, y = mean_odds_ratio, group = rank_method,
  ...
  labs(y = "Mean Odds Ratio", x = "Group Percentile",
       title = "T2D (Tie Method on RA): Comparison of Rank Methods Across Top Percentiles") +
  guides(color = guide_legend(override.aes = list(shape = c(19, 17, 15, 18, 16, 13))))


plot
ggsave("R6_TIE_T2D_OG_Top_Percentiles.png", plot = plot, width = 10, height = 8, dpi = 300)
