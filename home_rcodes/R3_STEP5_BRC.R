...
process_T2D_group <- function(group_number) {
  ...
  file_name <- sprintf("AGE_R3_T2D_group%d.csv", group_number)
  ...
  for(norm in norms) {
    norm_data <- read_csv(sprintf("skew-nuclear/matlab/R3_new_%s_T2D_group%d.csv", norm, group_number), col_names = FALSE)
  ...
  mc4_data <- read_csv(sprintf("new_R3_mc4_T2D_group%d.csv", group_number))
  mct_data <- read_csv(sprintf("new_R3_mct_T2D_group%d.csv", group_number))
  T2D_group$mc4_rank <- mc4_data$mc4_rank
  T2D_group$mct_rank <- mct_data$mct_rank

  write.csv(T2D_group, sprintf("AGE_R3_new_T2D_group%d_with_ranks.csv", group_number), row.names = FALSE)
