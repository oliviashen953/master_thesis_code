...
for (group_id in 1:20) {
  ...
  file_name <- sprintf("TIE_R6_new_Height_group%d_with_ranks.csv", group_id)
  ...
  if (file.exists(file_name)) {
    ...
    output_file_name <- sprintf("R6_Height_group%d_standardized_differences_and_rr.csv", group_id)
...
for (group_id in 1:20) {
  file_name <- sprintf("R6_Height_group%d_standardized_differences_and_rr.csv", group_id)
...
write_csv(summary_results, "R6_Height_summary_standardized_differences_and_rr_across_groups.csv")
