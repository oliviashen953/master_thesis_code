...
library(readr)
R4_T2D_summary_odds_ratio_and_rr_across_groups <- read_csv("R4_T2D_summary_odds_ratio_and_rr_across_groups.csv")
View(R4_T2D_summary_odds_ratio_and_rr_across_groups)

library(readr)
T2D_summary <- R4_T2D_summary_odds_ratio_and_rr_across_groups

ra_methods <- c("mc4_rank", "logs_rank", "RA_average_rank")
pgs_methods <- c("supervise3000_ols_rank_no", "PGS003443_rank", "pgs_average_rank")
...
pgs_labels <- setNames(c(
  ...
  "Supervised3000 Best PGS Model: PGS003443",
  "Average PGS Rank"
), all_methods)
...
method_colors <- c(
  ...
  "Supervised3000 Best PGS Model: PGS003443" = "#984EA3", # Purple
...
plot <- ggplot(filtered_methods, aes(x = percentile, y = mean_odds_ratio, group = rank_method,
  ...
  labs(y = "Mean Odds Ratio", x = "Group Percentile",
       title = "T2D (Supervised Older3000): Comparison of Rank Methods Across Top Percentiles") +
  guides(color = guide_legend(override.aes = list(shape = c(19, 17, 15, 18, 16, 13))))


plot
ggsave("R4_T2D_OG_Top_Percentiles.png", plot = plot, width = 10, height = 8, dpi = 300)
