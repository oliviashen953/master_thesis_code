...
process_BMI_group <- function(group_number) {
  ...
  file_name <- sprintf("AGE_R4_BMI_group%d.csv", group_number)
  ...
  for(norm in norms) {
    norm_data <- read_csv(sprintf("skew-nuclear/matlab/R4_new_%s_BMI_group%d.csv", norm, group_number), col_names = FALSE)
  ...
  mc4_data <- read_csv(sprintf("new_R4_mc4_BMI_group%d.csv", group_number))
  mct_data <- read_csv(sprintf("new_R4_mct_BMI_group%d.csv", group_number))
  BMI_group$mc4_rank <- mc4_data$mc4_rank
  BMI_group$mct_rank <- mct_data$mct_rank
  

  write.csv(BMI_group, sprintf("AGE_R4_new_BMI_group%d_with_ranks.csv", group_number), row.names = FALSE)
