...
for (group_id in 1:20) {
  file_name <- sprintf("AGE_R4_new_BrC_group%d_with_ranks.csv", group_id)
  ...
  if (file.exists(file_name)) {
    ...
    # Calculate OR & RR for each rank method and each top percentile
    ...
    output_file_name <- sprintf("R4_BrC_group%d_odds_ratio_and_rr.csv", group_id)
