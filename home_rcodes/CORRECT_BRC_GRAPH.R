...
pgs_methods <- c("supervise3000_ols_rank_no", "PGS000539_rank", "pgs_average_rank")
...
pgs_labels <- setNames(c(
  ...
  "Supervised3000 Best PGS Model: PGS000539",
  "Average PGS Rank"
), all_methods)
...
method_colors <- c(
  ...
  "Supervised3000 Best PGS Model: PGS000539" = "#984EA3", # Purple
...
plot <- ggplot(filtered_methods, aes(x = percentile, y = mean_odds_ratio, group = rank_method,
  ...
  labs(y = "Mean Odds Ratio", x = "Group Percentile",
       title = "BrC: Comparison of Rank Methods Across Top Percentiles") +
  guides(color = guide_legend(override.aes = list(shape = c(19, 17, 11, 15, 18, 16, 13))))


plot
ggsave("AMS_INCLUDED_BrC_OG_Top_Percentiles.png", plot = plot, width = 10, height = 8, dpi = 300)
