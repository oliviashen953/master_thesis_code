...
pgs_benchmarks <- c("supervise3000_ols_rank_no", "pgs_avg_rank", "pgs_median_rank", "PGS000214_rank", "PGS000497_rank")
...
ggplot(Top_percentiles, aes(x = percentile, y = percentage_pheno_1s, group = rank_method,
  ...
  labs(y = "percentile", x = "percentile",
       title = "BrC Group 1:Comparison of RA Methods for Top percentiles") +
  ylim(y_min, y_max) 


ggsave("Supervised3000_test_new_BrC_group12_Top_percentiles.png", width = 10, height  = 8, dpi = 300)
...
p <- ggplot(filtered_methods, aes(x = percentile, y = percentage_pheno_1s, group = rank_method)) +
  ...
  labs(title = "Supervised3000_BrC group12: Comparison of RA Methods for all Percentiles",
       x = "Percentile",
...
ggsave("Supervised3000_new_BrC_group12_ALL_percentiles.png", plot = p, width = 12, height = 8, dpi = 300)
