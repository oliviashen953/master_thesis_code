...
library(forcats) 
library(readr)
T2D_summary <- read_csv("R4_T2D_summary_odds_ratio_and_rr_across_groups.csv")
...
method_colors <- c(
  ...
  "Supervised_Best: PGS003443" = "#984EA3", # Purple
...
plot <- ggplot(filtered_methods, aes(x = percentile, y = mean_odds_ratio, group = rank_method,
  ...
  #title = "T2D (Supervised AFR3000): Comparison of Rank Methods Across Top Percentiles") +
  guides(color = guide_legend(override.aes = list(shape = c(19, 17, 15, 18, 16, 13))))


plot
ggsave("r4_T2D_Top_Percentiles.png", plot = plot, width = 10, height = 8, dpi = 300)
