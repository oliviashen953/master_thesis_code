...
library(readr)
Sorted_Summary_BrC_averages_pheno_by_percentile <- read_csv("SORTED_Summary_BrC_percentage_pheno_1s.csv")
View(Sorted_Summary_BrC_averages_pheno_by_percentile)

BrC_TOTOAL_results <- Sorted_Summary_BrC_averages_pheno_by_percentile


library(dplyr)


pgs_benchmarks <- c("supervise3000_african_ols_rank_no","pgs_average_rank", "PGS000503_rank")
ra_methods <- c("logs_rank", "mc4_rank")


all_methods <- c(pgs_benchmarks, ra_methods)


filtered_methods <- BrC_TOTOAL_results %>%
  filter(rank_method %in% all_methods)

pgs_labels <- c(
  "supervise3000_african_ols_rank_no" = "supervised OLS 3000 training (supervise AFR3000)",
  "PGS000503_rank" = "supervised best 3000 training: PGS000503_rank (supervise AFR3000)",
  "logs_rank" = "Best RA method: Nuclear Norm logs_rank",
...
ggplot(top_percentiles, aes(x = percentile, y = mean_percentage_pheno_1s, group = rank_method,
  ...
  labs(y = "Average Percentage of Pheno == 1", x = "Top Catching Percentiles",
       title = "BrC:Comparison of Best Two RA Methods with PGS Models for Top percentiles (supervise AFR3000)") +
  ylim(y_min, y_max) # Apply the focused y-axis limits


ggsave("MC4_ADD_R1_AFR_BrC_Top_Percentiles.png", width = 10, height = 8, dpi = 300)
