...
pgs_methods <- c("real_african_ols_rank_no", "PGS002161_rank", "pgs_average_rank")
...
pgs_labels <- setNames(c(
  ...
  "Supervised AFR3000 Best PGS Model: PGS002161",
  "Average PGS Rank"
), all_methods)
...
method_colors <- c(
  ...
  "Supervised AFR3000 Best PGS Model: PGS002161" = "#984EA3", # Purple
...
plot <- ggplot(filtered_methods, aes(x = group_percentile, y = mean_standardized_difference, group = rank_method,
  ...
  labs(y = "Mean Standardized Difference", x = "Group Percentile",
       title = "BMI (Supervised AFR3000): Comparison of Rank Methods Across Top Percentiles") +
  guides(color = guide_legend(override.aes = list(shape = c(19, 17, 15, 18, 16, 13))))


plot
ggsave("BMI_AFR_Top_Percentiles.png", plot = plot, width = 10, height  = 8, dpi = 300)
