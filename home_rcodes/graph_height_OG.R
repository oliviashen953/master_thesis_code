...
pgs_benchmarks <- c("real_ols","pgs_average", "PGS001943")
ra_methods <- c("mc4")
...
pgs_labels <- c(
  "real_ols" = "supervised OLS 3000 training",
  "PGS001943" = "supervised best 3000 training: PGS001943",
...
ggplot(top_percentiles, aes(x = percentile, y = mean_average_pheno, group = rank_method,
  ...
  theme_bw() +
  labs(y = "Average BMI", x = "Top Catching Percentiles",
       title = "BMI: Comparison of Best RA Method with PGS Models for Top percentiles") +
  ylim(y_min, y_max)

ggsave("R1_BMI_Top_Percentiles.png", width = 10, height = 8, dpi = 300)
