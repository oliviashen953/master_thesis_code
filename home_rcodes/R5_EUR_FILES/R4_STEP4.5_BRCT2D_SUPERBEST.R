...
pgs_columns <- grep("PGS", names(BrC_EUR_supervised_sample), value = TRUE)
...
for (pct in percentiles) {
  ...
  for(pgs in pgs_columns) {
    ...
    threshold <- quantile(BrC_EUR_supervised_sample [[pgs]], (100-pct)/100, na.rm = TRUE)

    top_pct_subjects <- BrC_EUR_supervised_sample [BrC_EUR_supervised_sample [[pgs]] > threshold, ]
...
for (pct in names(results)) {
  ...
  cat("BrC SUPERVISED TWENTY: For the top", pct, ", the best-performing PGS model is:", best_model_name, 
