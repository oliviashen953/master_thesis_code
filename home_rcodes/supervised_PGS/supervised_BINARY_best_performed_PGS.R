...
#####################################################################################################################
#####################################################################################################################
pgs_columns <- grep("PGS", names(BrC_65_supervised_sample), value = TRUE)
...
for (pct in percentiles) {
  ...
  for(pgs in pgs_columns) {
    ...
    threshold <- quantile(BrC_65_supervised_sample [[pgs]], (100-pct)/100, na.rm = TRUE)
    

    top_pct_subjects <- BrC_65_supervised_sample [BrC_65_supervised_sample [[pgs]] > threshold, ]
