...
pgs_benchmarks <- c("supervise3000_ols_rank_no","pgs_average_rank", "PGS000007_rank")
...
pgs_labels <- c(
  ...
  "PGS000007_rank" = "supervised best 3000 training: PGS000007",
...
ggplot(top_percentiles, aes(x = percentile, y = mean_percentage_pheno_1s, group = rank_method,
  ...
  labs(y = "Average Percentage of Pheno == 1", x = "Top Catching Percentiles",
       title = "BrC: Comparison of Best Two RA Methods with PGS Models for Top percentiles") +
  ylim(y_min, y_max)


ggsave("MC4_ADD_R1_BrC_Top_Percentiles.png", width = 10, height = 8, dpi = 300)
