...
BMI_summary <- read_csv("R6_BMI_summary_standardized_differences_and_rr_across_groups.csv")


ra_methods <- c("mc4_rank", "logs_rank","RA_average_rank")
pgs_methods <- c("supervise3000_ols_rank_no", "PGS002161_rank", "pgs_average_rank")
...
pgs_labels <- setNames(c(
  ...
  "Supervised3000 Best PGS Model: PGS002161",
  "Average PGS Rank"
), all_methods)
...
method_colors <- c(
  ...
  "Supervised3000 Best PGS Model: PGS002161" = "#984EA3", # Purple
...
plot <- ggplot(filtered_methods, aes(x = group_percentile, y = mean_standardized_difference, group = rank_method,
  ...
  labs(y = "Mean Standardized Difference", x = "Group Percentile",
       title = "BMI (Tie Method on RA): Comparison of Rank Methods Across Top Percentiles") +
  guides(color = guide_legend(override.aes = list(shape = c(19, 17, 15, 18, 16, 13))))


plot
ggsave("R6_TIE_BMI_OG_Top_Percentiles.png", plot = plot, width = 10, height = 8, dpi = 300)
