...
pgs_labels <- setNames(c(
  ...
  "AVG_RA_Rank",
  "Supervised_OLS",
  "Supervised_Best: PGS002313_rank",
  "AVG_PRS"
), all_methods)
...
method_colors <- c(
  ...
  "Supervised_Best: PGS002313_rank" = "#984EA3", # Purple
...
plot <- ggplot(filtered_methods, aes(x = group_percentile, y = mean_standardized_difference, group = rank_method,
  ...
  # title = "BMI: Comparison of Rank Methods Across Top Percentiles") +
  guides(color = guide_legend(override.aes = list(shape = c(19, 17, 15, 18, 16, 13))))


plot
ggsave("r4_older_BMI_Top_Percentiles.png", plot = plot, width = 10, height = 8, dpi = 300)
