...
for (group_id in 1:20) {
  file_name <- sprintf("TIE_R6_new_BrC_group%d_with_ranks.csv", group_id)
  ...
  if (file.exists(file_name)) {
    ...
    output_file_name <- sprintf("R6_BrC_group%d_odds_ratio_and_rr.csv", group_id)
...
for (group_id in 1:20) {
  file_name <- sprintf("R6_BrC_group%d_odds_ratio_and_rr.csv", group_id)
  if (file.exists(file_name)) {
...
write_csv(summary_results, "R6_BrC_summary_odds_ratio_and_rr_across_groups.csv")
